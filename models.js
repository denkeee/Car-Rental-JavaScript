...
async function renderModels() {
  try {
    ...
    const modelsFilter = document.querySelector("#models-filter").value;

    modelsList.innerHTML = `<i class="fa-solid fa-spinner models__list__spinner"></i>`;

    const models = await fetchModels();
    if (modelsFilter === "RATING") {
      models.sort((a, b) => {
        return b.rating - a.rating;
      });
    } else if (modelsFilter === "LOW_TO_HIGH") {
      models.sort((a, b) => {
        return a.per_day_price - b.per_day_price;
      });
    } else if (modelsFilter === "HIGH_TO_LOW") {
      models.sort((a, b) => {
        return b.per_day_price - a.per_day_price;
      });
    }
      ...
      .map((model) => {
        ...
        <div class="model__details model__details-1">
          ...
          <h4 class="model__details__price">
            $${model.per_day_price} <span class="model__details__price__span">per day</span>
          </h4>
        </div>
        <div class="model__details model__details-2">
          ...
          <div class="model__detail">
            <i class="model__detail__icon fa-solid fa-car"></i>
            <span class="model__detail__text">${model.model}</span>
          </div>
          <div class="model__detail model__detail-right">
            <i class="model__detail__icon fa-solid fa-car"></i>
